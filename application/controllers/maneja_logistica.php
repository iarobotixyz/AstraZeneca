...
class Maneja_logistica extends CI_Controller {
...
///////Cargar Usuarios activos para asignar en el listado
...
///////////////////////////////////////////
      $cervezas=$this->input->post('personal');
    //recorremos el array de cervezas seleccionadas. No olvidarse q la primera posición de un array es la 0 
          ...
          if ($cervezas=='') 
          {
            //echo "notrae dato significa que no se asigno ningun personal";
          }
          if ($cervezas!='') 
          {
            for ($i=0;$i<count($cervezas);$i++) 
            { 
              echo "Procesando Informacion: Incompleta";
            //echo "trae dato, asignado a algun personal.";
            echo "<br>" . $i . ": " . $cervezas[$i]; 
            }//fin del for 
          }
...
$ligadel="'borrar?id=".$idlog."'";

$crud='<br><input style="border: #98141b 2px solid; color: #fff; background-color: #cd1920" type="image" src="images/del.jpg" width="25" height="25" value="Borrar" onclick="location.href='.$ligadel.'"/>';//
//$crud = $IDlog.'B';
...
//////////////////////////////////////////////////////////
           ...
           $this->table->add_row($selec, $tablaFolio,$tablaCuestionario,$tablaCliente,$tablaDireccion,$UsuarioActivo.$crud,$tablaFechas,$array[14]);//8
}//fin archivo
}//fin del for
...
//<input type="button" value="Borrar" onclick="borrarLogistica()"/>
            ...
            $datos_Dispos = $this->modelo_mv_logistica->listaidlog($cuentaId);
            if ($datos_Dispos=='') 
            {
              $data['datosDispositivos']='SIN DATOS';//datosDispositivos esta parte es para arreglar el primer dispo
            }
            if ($datos_Dispos!='') 
            {
             $data['datosDispositivos']=$datos_Dispos;//datosDispositivos
            }
...
//Para archivar es necesario ponerlo como no activo en la tabla de logistica, pero tambien es eliminarlo de la parte de dispositivos, o ponerlo como no disponible.
      ...
      if ($idd!='') 
      {
        $this->modelo_mv_logistica->borrar_logistica($cuentaId,$idd);
      }
      redirect(base_url().'mostrarLogistica');
    }
