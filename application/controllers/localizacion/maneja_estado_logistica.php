...
class maneja_estado_logistica extends CI_Controller {
	...
	public function e_d($primer=FALSE){
		...
		$this->load->view('vista_mv/EstadoLogistica');
			...
			if($listaGps!=0)
{
        ...
        //Primero pedir el datoseventos del paquete por si tiene rastreador propio con su propio ID
...
//para consultar el nombre del dispositivo que estamos usando.
$cuentaId= $this->session->userdata('cuenta');
        $datt= array();
        $datt=$this->modelo_mv_logistica_status->pasign($cuentaId,$array[1]);
        $printPasign=$datt['mostrarNombre'];
//fin consulta nombre de dispositivo

							$direccion='<table border="0">
							

							<tr><td><center><b>Fecha de Envio</b></center></td></tr>
							<tr><td><center>'.$ufecha.'</center></td></tr>	

							<tr><td><center><b>Personal Asignado</b></center></td></tr>
							<tr><td><center>'.$printPasign.'</center></td></tr>	





							<tr><td><center><b>Direccion actual del Personal</b></center></td></tr>
							<tr><td><center>'.$datos['direccion'].'</center></td></tr>
							<tr><td><center><b>Dispositivo</b></center></td></tr>		
							<tr><td><center>'.$array[35].'</center></td></tr>
...
$tformulario='<table border="2">
				
				<tr>
					<td><center><b>'.$array[36].'</b><br><u>'.$array[5].'</u></center></td>
					<td>'.$imagenONOFF1.'</td>
					<td>'.$estado1.'</td>
				</tr>
				<tr>
					<td><center><b>'.$array[37].'</b><br><u>'.$array[6].'</u></center></td>
					<td>'.$imagenONOFF2.'</td>
					<td>'.$estado2.'</td>
				</tr>
				<tr>
					<td><center><b>'.$array[38].'</b><br><u>'.$array[7].'</u></center></td>
					<td>'.$imagenONOFF3.'</td>
					<td>'.$estado3.'</td>
				</tr>
				<tr>
					<td><center><b>'.$array[39].'</b><br><u>'.$array[8].'</u></center></td>
					<td>'.$imagenONOFF4.'</td>
					<td>'.$estado4.'</td>
				</tr>
				<tr>
					<td><center><b>'.$array[40].'</b><br><u>'.$array[9].'</u></center></td>
					<td>'.$imagenONOFF5.'</td>
					<td>'.$estado5.'</td>
				</tr>

								<tr>
					<td><center><b>'.$array[41].'</b><br><u>'.$array[10].'</u></center></td>
					<td>'.$imagenONOFF6.'</td>
					<td>'.$estado6.'</td>
				</tr>
				<tr>
					<td><center><b>'.$array[42].'</b><br><u>'.$array[11].'</u></center></td>
					<td>'.$imagenONOFF7.'</td>
					<td>'.$estado7.'</td>
				</tr>
				<tr>
					<td><center><b>'.$array[43].'</b><br><u>'.$array[12].'</u></center></td>
					<td>'.$imagenONOFF8.'</td>
					<td>'.$estado8.'</td>
				</tr>
				<tr>
					<td><center><b>'.$array[44].'</b><br><u>'.$array[13].'</u></center></td>
					<td>'.$imagenONOFF9.'</td>
					<td>'.$estado9.'</td>
				</tr>
				<tr>
					<td><center><b>'.$array[45].'</b><br><u>'.$array[14].'</u></center></td>
					<td>'.$imagenONOFF10.'</td>
					<td>'.$estado10.'</td>
				</tr>
...
<td width="50%"></td>
	...
	<input style="width : 300px; heigth : 300px border: #000 4px solid; color: #fff; background-color: #228B22" type="button" value="Abierto" />';
}
if ($array[4]==1) {
	//$liga="'archivar?id='";
...
if ($array[4]==3) {
	...
	$Status='<input style="border: #98141b 2px solid; color: #fff; background-color: #cd1920" type="button" value="Cerrado"/><br><input style="border: #98141b 2px solid; color: #fff; background-color: #cd1920" type="button" value="Archivar" onclick="location.href='.$liga.'"/>';
}
...
$this->table->add_row($cont,$nCuestionario,$tformulario,$direccion);
					            $cont=$cont+1;
            ...
            $header = array('#','Nombre de Cuestionario','Cuestionario','Detalles');
            //Establece los títulos 
            $this->table->set_heading($header);
            //formato de tabla 
            $tmpl = array (
                    'table_open'          => '<article><section class="reportedesde"><p><br><table border="2" cellpadding="1" align="center" cellspacing="0" width="100%">',
            ...
            $this->table->set_template($tmpl);
		$this->load->view('vista_mv/EstadoLogistica');
		echo "<table align='center'><tr><td>Estado del Formulario ";
		$hora=date('h:i:s',time());//date('d/m/Y')
		echo "<font size='2'>".$hora."</font></td></tr></table>";
	}
